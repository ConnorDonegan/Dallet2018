...
for(i in seq_along(alphas$wardx)) {
  ...
  sc[which(sc$ward == wardx), 5:ncol(sc)] <- NA
  sc[which(sc$ward == wardx), 5:(4+length(wards))] <- wards
...
sc_tidy <- sc %>%
  gather(key = delete.me, value = STR_WARDS, -c(county, ward_name, ward, CTV, dallet_pct)) %>% 
  as.tibble() %>%
...
sc_tidy[ , c(1,3:5)] <- map_df(sc_tidy[, c(1,3:5)], standardize_names)
sc_tidy <- sc_tidy[,c('CNTY_NAME', 'NAME', 'STR_WARDS', 'CTV', 'dallet_pct')] %>%
  as.tibble()
...
wi <- merge(wi, munis, by = 'GEOID', all.x = T)
wi.names <- which(names(wi) %in% c('NAME', 'CNTY_NAME', 'STR_WARDS', 'CTV'))
wi[ , wi.names] <- map_df(wi[ , wi.names], standardize_names)
